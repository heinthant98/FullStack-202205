...
public class TeacherService {
	...
	public List<TeacherListVO> search(Optional<String> name, Optional<String> phone, Optional<String> email) {
		...
		where.append(email.filter(StringUtils::hasLength).map(s -> {
			params.put("email", s.concat("%"));
			return "and a.email like :email ";
		}).orElse("")
		);
		
		where.append(phone.filter(StringUtils::hasLength).map(s -> {
			params.put("phone", s.concat("%"));
			return "and t.phone like :phone ";
		}).orElse("")
		);
		
		where.append(name.filter(StringUtils::hasLength).map(s -> {
			params.put("name", s.toLowerCase().concat("%"));
	...
	public List<IdWithName> getAvailableTeachers() {
		return template.query(
				"select t.id, a.name from teacher t join account a on a.id = t.id where deleted = :del",
				Map.of("del", false), 
